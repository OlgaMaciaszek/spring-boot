...
class UndertowWebServerRuntimeHintsTests {
	...
	void registersHints() throws ClassNotFoundException {
		...
		assertThat(RuntimeHintsPredicates.reflection().onFieldInvocation(Undertow.class, "listeners"))
			.accepts(runtimeHints);
		assertThat(RuntimeHintsPredicates.reflection().onFieldInvocation(Undertow.class, "channels"))
			.accepts(runtimeHints);
	...
	private Predicate<RuntimeHints> reflectionOnField(String className, String fieldName)
		...
		return RuntimeHintsPredicates.reflection()
			.onFieldInvocation(ReflectionUtils.findField(Class.forName(className), fieldName));
	}
